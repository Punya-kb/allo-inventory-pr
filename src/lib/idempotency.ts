...
type ConfirmBody = { reservation?: unknown; error?: string };

export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  const idempotencyKey = req.headers.get('Idempotency-Key');
  const { status, body, replayed } = await withIdempotency<ConfirmBody>(
    idempotencyKey,
    `POST /api/reservations/${params.id}/confirm`,
    async (): Promise<{ status: number; body: ConfirmBody }> => {
      try {
...
"@ | Set-Content -LiteralPath "src/app/api/reservations/[id]/confirm/route.ts" -Encoding UTF8